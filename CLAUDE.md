...
- **Language**: English is the main site (`/`), German is the mirror (`/de`)
...
- OFFER (v2 2026-08-20, same day the €490 Startanalyse was tried and killed — Danyal
  ...
  meeting, contract. DE headline: "Ihr kompletter Marketing-Plan. Kostenlos. Bevor Sie einen Cent ausgeben." 4-step line: buchen -> verstehen -> arbeiten -> Plan. Copy deliberately SHORT and simple — Danyal rejected longer/technical versions twice. FUNNEL (2026-08-20): HERO FORM REMOVED -> Calendly booking card; bottom form is the ONLY form; callback contact card removed — calls go through Calendly. WhatsApp contact card kept. FORM FIELDS (updated 2026-09-09 — user directive): Phone and Email are BOTH required (`name="telefon"` type="tel", `name="email"` type="email"), same row, DE+EN. `validPhone()`/`validEmail()` JS validators run on submit (both files, ~line 813 EN / ~847 DE) alongside the required-field check. This reverses the 2026-08-20 email-only decision — phone is back and required. DESIGN: animated glow orbs (hero + offer card), h1 2.9rem / section h2 2.15rem, scroll-reveal with 3s failsafe (never leave .rv unrevealed).
